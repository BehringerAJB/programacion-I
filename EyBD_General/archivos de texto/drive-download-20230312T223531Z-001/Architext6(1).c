...
//  Cuenta las palabras por renglones y determina cual es la palabra más larga
void mostrar(FILE*);
void cargar(FILE*);
int contar(char[]);
int error(char*);
...
{
 FILE *archivo;
 char nom1[80],cad[80], palmax[80];// nombre físico del archivo
 int j,cuenta;
 system("cls");
...
while(fgets(cad,81,archivo)!=NULL)
{
   cuenta=contar(cad);
   larga(cad,palmax);
   p("\n En el renglon %d cantidad de palabras:%d y la palabra mas larga es: %s con %d caracteres ",++j,cuenta,palmax,dim(palmax));
}
