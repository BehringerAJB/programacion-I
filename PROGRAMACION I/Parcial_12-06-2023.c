/*
Una empresa muy importante del rubro del deporte 
nos contrató porque quiere un sistema para ayudar
 a contabilizar datos en los partidos de básquet
  de la liga nacional.
   para ello se deberá realizar un programa al que se le puede ir 
   ingresando los puntos (del equipo 1 o 2) y el valor del punto( 0,1,2,3). 
   El partido y la carga de información finalizará cuando se ingrese que el jugador pertenece al equipo 0. 
   Luego de cualquier carga de datos se deberá siempre ir actualizando el puntaje del partido 
   e indicar que equipo va ganando. Cuando el programa finalice, deberá indicar. 
...
* El porcentaje de puntos que se representan los tiros simples, dobles y triples sobre el total.
...
int main()
{
    ...
    int tsimples = 0, tdobles = 0, ttriples = 0; // suma de los tiros
    int ts = 0, td = 0, tt = 0; // totales por tiro
    int valor_punto;
